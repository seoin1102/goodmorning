...
//			}
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		String jwtToken = JWT.create().withSubject("sub")
				...
				.withClaim("email", principalDetails.getUserVo().getEmail()) // 원하는 값을 토큰에 넣을 수 있다
				.withClaim("name", principalDetails.getUserVo().getName())
				.withClaim("phoneNumber", principalDetails.getUserVo().getPhoneNumber())
				.withClaim("profileUrl", principalDetails.getUserVo().getProfileUrl())
				.withClaim("signUpDate", principalDetails.getUserVo().getSignUpDate())
				.withClaim("job", principalDetails.getUserVo().getJob())
				.withClaim("no", principalDetails.getUserVo().getNo()).sign(Algorithm.HMAC512("cos"));
